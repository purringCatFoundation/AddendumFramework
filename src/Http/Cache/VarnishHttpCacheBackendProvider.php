...
final class VarnishHttpCacheBackendProvider implements HttpCacheBackendProvider
{
    public function supports(HttpCacheConfigurationInterface $configuration): bool
    {
        return $configuration instanceof VarnishHttpCache;
    }
    ...
    public function invalidate(
        ...
        return $config->context->withDebugHeaders($response, 'INVALIDATE', 'varnish')
    ...
    ): ResponseInterface
    {
        ...
        return $config->context->withDebugHeaders($response, 'MISS', 'varnish');
    }

    private function configuration(HttpCacheConfigurationInterface $configuration): VarnishHttpCache
    {
        if (!$configuration instanceof VarnishHttpCache) {
            throw new \InvalidArgumentException('Varnish HTTP cache configuration expected');
        }
